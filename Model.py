...
class Model:
    ...
    def validate(self, X: Optional[List[List[Optional[int]]]], y: Optional[List[List[Optional[int]]]]) -> int:
        ...
        for validate_sample_X, validate_sample_y in zip(X, y):
            ...
            print("Sample:", validate_sample_X, "\tPrediction:", prediction.T, "\tActual:",
                  validate_sample_y, "\tMax Pred:", max_index_prediction, "\tMax Actual:", max_index_y)
    ...
    def backward(self, y: Optional[List[List[Optional[int]]]]) -> None:
        ...
        """
        self.layers[-1].error = self.loss.backward(
            self.layers[-1].z, y, self.layers[-1].a)

        # Loop backwards, skipping the last layer due to 
        # its different error function
